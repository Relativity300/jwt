...
const provedor = require('../models/provedores.models')
...
const IdentificationExisting = async ( identificacion = '') =>{
    const documentExisting = await provedor.findOne({identificacion});
    if(documentExisting){
...
const EmailExisting = async ( email = '') =>{
    const emailExisting = await provedor.findOne({email});
    if(emailExisting){
